...
const MultiSelect = ({
  ...
  // Find selected options based on value array
  useEffect(() => {
    if (value && Array.isArray(value) && options.length > 0) {
      const found = options.filter(option => value.includes(option.value));
        ...
        <PopoverTrigger>
          <Box w="full">
            <Button
              ...
              borderColor={error ? 'red.300' : isOpen ? 'purple.400' : 'gray.300'}
              borderWidth="1px"
              borderRadius="lg"
              _hover={{ 
                borderColor: error ? 'red.400' : isOpen ? 'purple.500' : 'gray.400',
                bg: isDisabled ? 'gray.50' : 'white',
                transform: 'translateY(-1px)',
                boxShadow: '0 4px 12px rgba(0, 0, 0, 0.1)',
              }}
              _focus={{ 
                borderColor: error ? 'red.500' : 'purple.400', 
                boxShadow: error 
              ...
              _before={{
                ...
                right: 0,
                bottom: 0,
                background: 'linear-gradient(135deg, rgba(147, 51, 234, 0.02) 0%, rgba(168, 85, 247, 0.02) 100%)',
                opacity: isOpen ? 1 : 0,
